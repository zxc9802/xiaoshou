...
app.patch('/api/v1/knowledge/:id', async (request) => { const current = actor(request); return knowledge.update(current.organizationId, (request.params as { id: string }).id, request.body as Record<string, unknown>, current.userId); });
